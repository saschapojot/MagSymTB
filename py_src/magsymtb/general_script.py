...
def get_atom_sort_key(atom):
    """
    Helper function to generate a sort key for atomIndex objects.
    Sorts primarily by natural order of wyckoff_instance_id (e.g., 'C0_2' before 'C0_10'),
    and secondarily by Cartesian coordinates.
    Args:
        atom: atomIndex object

    Returns:

    """
    # Parse wyckoff_instance_id for natural sorting (e.g., 'Ca0_1' -> ('Ca', 0, 1))
    match = re.match(r'^([A-Za-z]+)(\d+)_(\d+)$', atom.wyckoff_instance_id)
    if match:
        w_key = (match.group(1), int(match.group(2)), int(match.group(3)))
    else:
        # Raise an error if the format doesn't match
        raise ValueError(f"Invalid wyckoff_instance_id format: '{atom.wyckoff_instance_id}'. Expected format like 'Ca0_1'")

    # Return tuple of (parsed_id, fractional_coordinates_tuple)
    # Tuples are sorted element-by-element automatically by Python
    # print(f"(w_key, tuple(atom.cart_coord))={(w_key, tuple(atom.cart_coord))}")
    return (w_key, tuple(atom.cart_coord))
...
def generate_atoms_in_unit_cell(parsed_config,magnetic_space_group_cart_spatial, lattice_basis,origin_cart,
    ...
    # ==============================================================================
    # Sort the unit_cell_atoms list using the standalone helper function
    # ==============================================================================
    # print(f"before sorting: unit_cell_atoms={unit_cell_atoms}")
    unit_cell_atoms.sort(key=get_atom_sort_key)

    # print(f"after sorting: unit_cell_atoms={unit_cell_atoms}")
...
def compute_dist(center_atom, unit_cell_atoms, radius, search_range=10):
    ...
    Returns:
        list: atomIndex objects within the specified radius, sorted by distance, and get_atom_sort_key
    ...
    # Sort by atom sort key  (primary) and then by distance (secondary)
    #distance
    neighbor_atoms.sort(key=lambda x: ( get_atom_sort_key(x[1]),x[0]))
...
def search_one_equivalent_atom(seed_atom,target_cart_coord, neighbor_atoms_copy, tolerance=1e-3):
    ...
    """
    # Iterate through all neighbor atoms in the list
...
def get_equivalence_classes_for_one_center_atom(center_atom_idx, unit_cell_atoms, all_neighbors,
    ...
    - neighbor_atoms_copy is a list  of references to DEEP-COPIED atomIndex objects
    - seed_atom = list .pop() returns a reference to one of these copied objects
    ...
    while len(neighbor_atoms_copy) != 0:
        ...
        for operation_idx in range(len(magnetic_space_group_cart_spatial)):
            ...
            if result[0]  is not None:
                ...
                # ==============================================================================
                # Search for matching neighbor in the remaining unclassified list
                # ==============================================================================
                ...
                #
                # This reference is ESSENTIAL for  list.remove() to work:
                # - Python's list.remove() uses object identity (memory address)
                # - We need the EXACT SAME object reference that's in the list
                ...
                # ==============================================================================
                # If we found a matching neighbor in the remaining list
        ...
        # x[0] is neighbor_atom_reference, x[2] is n_vec
        current_equivalence_class.sort(
            key=lambda x: (get_atom_sort_key(x[0]), tuple(x[2]))
                )
...
def generate_all_trees_for_unit_cell(unit_cell_atoms,all_neighbors,magnetic_space_group_cart_spatial,spinor_mat_representation,delta_vec,identity_idx,type_linear,parsed_config,tolerance=1e-3):
    ...
    for i, center_atom_i in enumerate(unit_cell_atoms):
        ...
        # ==============================================================================
        equivalence_classes_center_atom_i = get_equivalence_classes_for_one_center_atom(
            center_atom_idx=i,
...
def run_general_computation(confFileName):
    ...
    for i, unit_atom in enumerate(unit_cell_atoms):
        ...
        # Reverse the order of neighbors before storing
        neighbors.reverse()
